...
# Quickview
data <- data %>% 
  select(Label, Condition, Pyruv, Trel, Flux, D13C, Qc_iso, Qc_flux, AtF)
...
AUC_summary_wide <- AUC_summary.CO2 %>%
  select(-c(AUC_name, perc.mass_name,mass.13C.g_name )) %>%
...
plot
ggsave("../../Figures/CO2-VOCs/total-C-CO2-flux.png",units=c('in'),width=4, height = 4,dpi=300,plot)
...
wilcox.test(mass.13C.g ~ Pyr, data = AUC.PD) #4.11e-5

# C1 vs C2 during drought
AUC.D <- AUC %>%
  filter(Cond == "Drought")

wilcox.test(mass.13C.g ~ Pyr, data = AUC.D) #0.00399

# pre-drought vs drought (C1)
AUC.C1 <- AUC %>%
  filter(Pyr == "C1")

wilcox.test(mass.13C.g ~ Cond, data = AUC.C1) #4.94e-4

# pre-drought vs drought (C2)
AUC.C2 <- AUC %>%
  filter(Pyr == "C2")

wilcox.test(mass.13C.g ~ Cond, data = AUC.C2) #2.76e-3
...
plot <- AUC.aton %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot
ggsave("../../Figures/CO2-VOCs/total-C-acetone-flux-C2.png",units=c('in'),width=2, height = 4,dpi=300,plot)
...
wilcox.test(mass.13C.g ~ Pyr, data = AUC.aton.PD) #0.0002879

# C1 vs C2 during drought
AUC.aton.D <- AUC.aton %>%
  filter(Cond == "Drought")

wilcox.test(mass.13C.g ~ Pyr, data = AUC.aton.D) #4.114e-05

# pre-drought vs drought (C1)
AUC.aton.C1 <- AUC.aton %>%
  filter(Pyr == "C1")

wilcox.test(mass.13C.g ~ Cond, data = AUC.aton.C1) #0.5457

# pre-drought vs drought (C2)
AUC.aton.C2 <- AUC.aton %>%
  filter(Pyr == "C2")

wilcox.test(mass.13C.g ~ Cond, data = AUC.aton.C2) #8.227e-05
...
for (i in 1:nrow(data.13.P.long.diac)) {
  if (isTRUE(data.13.P.long.diac[i,1] == data.13.P.long.diac[(i+1),1]) == TRUE) 
  {df.p.0.diac[i,] <- spread(data.13.P.long.diac[i:(i+1),], Time, Time_Flux)[1,]
  }
}
...
for (i in 1:nrow(data.13.D.long.diac)) {
  if (isTRUE(data.13.D.long.diac[i,1] == data.13.D.long.diac[(i+1),1]) == TRUE) 
  {df.d.0.diac[i,] <- spread(data.13.D.long.diac[i:(i+1),], Time, Time_Flux)[1,]
  }
}
...
plot.Diac <- AUC.Diac %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot.Diac
ggsave("../../Figures/CO2-VOCs/total-C-diacetyl-flux.png",units=c('in'),width=6, height = 4,dpi=300,plot.Diac)
...
plot.Diac <- AUC.Diac %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot.Diac
ggsave("../../Figures/CO2-VOCs/C-diacetyl-perc-flux.png",units=c('in'),width=6, height = 4,dpi=300,plot.Diac)
...
plot.Diac <- AUC.Diac %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot.Diac
ggsave("../../Figures/CO2-VOCs/total-C-diacetyl-flux-c2.png",units=c('in'),width=2, height = 4,dpi=300,plot.Diac)
...
plot.Diac <- AUC.Diac %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot.Diac
ggsave("../../Figures/CO2-VOCs/C-diacetyl-perc-flux-c2.png",units=c('in'),width=4, height = 4,dpi=300,plot.Diac)
...
wilcox.test(mass.13C.g ~ Pyr, data = AUC.Diac.PD) #0.2973

# C1 vs C2 during drought
AUC.Diac.D <- AUC.Diac %>%
  filter(Cond == "Drought")

wilcox.test(mass.13C.g ~ Pyr, data = AUC.Diac.D) #0.7304

# pre-drought vs drought (C1)
AUC.Diac.C1 <- AUC.Diac %>%
  filter(Pyr == "C1")

wilcox.test(mass.13C.g ~ Cond, data = AUC.Diac.C1) #0.2224

# pre-drought vs drought (C2)
AUC.Diac.C2 <- AUC.Diac %>%
  filter(Pyr == "C2")

wilcox.test(mass.13C.g ~ Cond, data = AUC.Diac.C2) #0.05031
```
...
plot <- AUC.acetate %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot
ggsave("../../Figures/CO2-VOCs/total-C-acetate-flux.png",units=c('in'),width=4, height = 4,dpi=300,plot)
...
plot <- AUC.acetate %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot
ggsave("../../Figures/CO2-VOCs/C-acetate-perc-flux.png",units=c('in'),width=6, height = 4,dpi=300,plot)
```
...
wilcox.test(mass.13C.g ~ Pyr, data = AUC.acetate.PD) #0.6665

# C1 vs C2 during drought
AUC.acetate.D <- AUC.acetate %>%
  filter(Cond == "Drought")

wilcox.test(mass.13C.g ~ Pyr, data = AUC.acetate.D) #0.7962

# pre-drought vs drought (C1)
AUC.acetate.C1 <- AUC.acetate %>%
  filter(Pyr == "C1")

wilcox.test(mass.13C.g ~ Cond, data = AUC.acetate.C1) #0.002756

# pre-drought vs drought (C2)
AUC.acetate.C2 <- AUC.acetate %>%
  filter(Pyr == "C2")

wilcox.test(mass.13C.g ~ Cond, data = AUC.acetate.C2) #0.0001645
```
...
summary.2 <- AUC_summary.CO2 %>%
  select(-mass.13C.g_name) %>% #remove column that doesn't match with diacetyl summary table
...
summary.umol <- summary.total %>%
  select(-c(AUC_name)) %>% #remove columns that we don't want to include in final summary table
...
# calculate perc totals across pre-drought and drought separately, then substract each from 100 to get 'other', which could be either other places 13C went or unmetabolized 13C-pyruvate
summary.drought <- summary.umol %>%
  filter(Cond == "Drought") 

d.other <- 100 - sum(summary.drought$sum.perc)

summary.predrought <- summary.umol %>%
  filter(Cond == "Pre_Drought") 

pd.other <- 100 - sum(summary.predrought$sum.perc)


df.diff <- data.frame(Cond = c("Drought",  "Pre_Drought"),
                      compound = c("other", "other"),
                      sum.perc = c(d.other, pd.other))

summary.with.other <- rbind(summary.umol, df.diff)

pie_D <- summary.with.other %>%
  filter(Cond == "Drought") %>%
  ggplot(aes(x="", y=sum.perc, fill = compound)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y", start = 0) +
   scale_fill_manual(breaks = c("CO2", "acetae", "Acetone", "Diacetyl", "other"),
                    labels = c(expression("CO"[2]*""), "Acetate", "Acetone", "Diacetyl", "Other"),
                    values = c("turquoise", "blue", "red", "yellow", "gray"))+
  theme_void()
pie_D
ggsave("../../Figures/CO2-VOCs/pie_D.png",units=c('in'),width=3, height = 3,dpi=300,pie_D)

pie_PD <- summary.with.other %>%
  filter(Cond == "Pre_Drought") %>%
  ggplot(aes(x="", y=sum.perc, fill = compound)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y", start = 0) +
...
pie_PD
ggsave("../../Figures/CO2-VOCs/pie_PD.png",units=c('in'),width=3, height = 3,dpi=300,pie_D)
...
summary.allocation.d <- summary.umol %>%
  ...
  select(perc.mass_name_C2, diff.perc, other)
...
pie.d <- ggplot(summary.allocation.d.t, aes(x="", y=perc.alloc, fill =cat.alloc )) +
 ...
 scale_fill_manual(breaks = c("diff.perc", "perc.mass_name_C2", "other"),
  ...
  theme(legend.title = element_blank())
pie.d
ggsave("../../Figures/CO2-VOCs/pie_d_bio_tca.png",units=c('in'),width=3, height = 3,dpi=300,pie.d)
...
pie.p <- ggplot(summary.allocation.p.t, aes(x="", y=perc.alloc, fill =cat.alloc )) +
  ...
  theme(legend.title = element_blank())
pie.p
ggsave("../../Figures/CO2-VOCs/pie_p_bio_tca.png",units=c('in'),width=3, height = 3,dpi=300,pie.p)

# make pie charts showing biosynthesis breakdown (this isn't right, need to subtract from biosyntheis percentage!!!)
# filter to just drought
summary.d <- summary.umol %>%
  filter(Cond == "Drought") 

# transpose to get vocs across rows for creating 'other.bio' row
summary.d.t <- t(summary.d)
colnames(summary.d.t) <- summary.d.t[2,]
summary.d.t <- summary.d.t[-(1:2),]
summary.d.t <- as.data.frame(summary.d.t)
summary.d.t$acetae <- as.numeric(summary.d.t$acetae)
summary.d.t$Acetone <- as.numeric(summary.d.t$Acetone)
summary.d.t$Diacetyl <- as.numeric(summary.d.t$Diacetyl)
summary.d.t$CO2 <- as.numeric(summary.d.t$CO2)

diff.perc.CO2 <- summary.d.t[7,3]


summary.d.t.bio <- summary.d.t %>%
  mutate(other = diff.perc.CO2 - (acetae + Acetone + Diacetyl)) %>%
  select(-CO2)

summary.d.t.bio.t <- t(summary.d.t.bio)
summary.d.t.bio.t <- as.data.frame(summary.d.t.bio.t)
summary.d.t.bio.t$compound <- rownames(summary.d.t.bio.t)


pie.d.bio <- ggplot(summary.d.t.bio.t, aes(x="", y = perc.mass_name_C2, fill =compound )) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y" , start = 0) +
 scale_fill_manual(breaks = c("acetae", "Acetone", "Diacetyl", "other"),
                    labels = c("Acetate", "Acetone","Diacetyl", "Other"),
                    values = c("lightblue", "green","yellow", "darkgray")) +
  theme_void() +
  theme(legend.title = element_blank())
pie.d.bio
ggsave("../../Figures/CO2-VOCs/pie_d_bio.png",units=c('in'),width=3, height = 3,dpi=300,pie.d.bio)


# pre-drought
# filter to just vocs and drought
summary.p <- summary.umol %>%
  filter(Cond == "Pre_Drought") 

# transpose to get vocs across rows for creating 'other.bio' row
summary.p.t <- t(summary.p)
colnames(summary.p.t) <- summary.p.t[2,]
summary.p.t <- summary.p.t[-(1:2),]
summary.p.t <- as.data.frame(summary.p.t)
summary.p.t$acetae <- as.numeric(summary.p.t$acetae)
summary.p.t$Acetone <- as.numeric(summary.p.t$Acetone)
summary.p.t$Diacetyl <- as.numeric(summary.p.t$Diacetyl)
summary.p.t$CO2 <- as.numeric(summary.p.t$CO2)

diff.perc.CO2 <- summary.p.t[7,3]


summary.p.t.bio <- summary.p.t %>%
  mutate(other = diff.perc.CO2 - (acetae + Acetone + Diacetyl)) %>%
  select(-CO2)

summary.p.t.bio.t <- t(summary.p.t.bio)
summary.p.t.bio.t <- as.data.frame(summary.p.t.bio.t)
summary.p.t.bio.t$compound <- rownames(summary.p.t.bio.t)


pie.p.bio <- ggplot(summary.p.t.bio.t, aes(x="", y = perc.mass_name_C2, fill =compound )) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y" , start = 0) +
 scale_fill_manual(breaks = c("acetae", "Acetone", "Diacetyl", "other"),
                    labels = c("Acetate", "Acetone","Diacetyl", "Other"),
  ...
  theme(legend.title = element_blank())
pie.p.bio
ggsave("../../Figures/CO2-VOCs/pie_p_bio.png",units=c('in'),width=3, height = 3,dpi=300,pie.p.bio)
