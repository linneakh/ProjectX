...
########C13 data###############
#Load 13C data
data.13.P <- read.csv("./Data/CO2-VOCs/voc_raw/diacetyl-pre-drought-13C.csv") 
data.13.D <- read.csv("./Data/CO2-VOCs/voc_raw/diacetyl-drought-13C.csv") 
...
plot <- ggplot(df.d.f, aes(x = Time, y = Flux, color = Pyr)) +
  #facet_grid(. ~ Pyruv) + 
  scale_x_continuous(breaks = c(-12, 0, 12, 24, 36, 48)) +
...
AUC <- AUC.0 %>%
  mutate(AUC.a = AUC - AUC.W.avg) %>% #Subtract natural abundance of 13C
  mutate(umol = ((AUC * 314.16) / 10000) * 1000) %>%
...
plot <- AUC %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot
ggsave("./Figures/CO2-VOCs/total-C-diacetyl-flux.png",units=c('in'),width=6, height = 4,dpi=300,plot)
...
plot <- AUC %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot
ggsave("./Figures/CO2-VOCs/C-diacetyl-perc-flux.png",units=c('in'),width=6, height = 4,dpi=300,plot)
...
plot <- AUC %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot
ggsave("./Figures/CO2-VOCs/total-C-diacetyl-flux-c2.png",units=c('in'),width=4, height = 4,dpi=300,plot)
...
plot <- AUC %>%
  ...
  theme(text = element_text(size = 12,
        ...
        legend.title = element_blank())
plot
ggsave("./Figures/CO2-VOCs/C-diacetyl-perc-flux-c2.png",units=c('in'),width=4, height = 4,dpi=300,plot)
