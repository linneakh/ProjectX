...
setwd("~/Documents/R/soil-pyruvate-metagenomics/deseq2/more_samples/only-missing-one-sample/WGCNA/power8-min30")
...
#vst.export <- read.csv("vst-metaT.csv", header = TRUE)
#rownames(vst.export) <- vst.export$X
#vst.export$X <- NULL

datExpr <- t(vst.export)
names(datExpr) <- rownames(vst.export)
...
clust = cutreeStatic(sampleTree, cutHeight = 180, minSize = 5)
table(clust)
...
traitData = read.csv("metaT-metadata-for-WGCNA-shorter.csv", header = TRUE)
traitData$Acetone.C2 <- as.numeric(traitData$Acetone.C2)
traitData$Acetate.C1 <- as.numeric(traitData$Acetate.C1)
traitData$Acetate.C2 <- as.numeric(traitData$Acetate.C2)
traitData$Diacetyl.C2 <- as.numeric(traitData$Diacetyl.C2)
traitData$NOSC <- as.numeric(traitData$NOSC)
...
MEs_order = MEs[,c(6,7,8,1,2, 9, 3,5,4)]
...
filename=paste("modules-trait-relationships-all-modules.png",sep="")
png(filename ,width=20, height=20,unit = "in", res = 1000)
labeledHeatmap(Matrix = moduleTraitCor,
               xLabels = c("Condition", "Time", "Acetate-c2", "Acetate-c1", 
                           "Acetone-c2", "Diacetyl-c2"),
               yLabels = c("BLA", "RED", "MAG", "TRQ", "GRN", "PNK", "BLU", "YLW", "BRN"),
...
#Erase grey column from MEs and only include modules related to drought:
MEs$MEgrey <- NULL

MEs_order = MEs[,c(6,7,8,3,5,4)]
...
#Display the correlation values within a heatmap plot
filename=paste("modules-trait-relationships.png",sep="")
png(filename ,width=20, height=20,unit = "in", res = 1000)
labeledHeatmap(Matrix = moduleTraitCor,
               xLabels = c("Condition", "Time", "Acetate-c2", "Acetate-c1", 
                           "Acetone-c2", "Diacetyl-c2"),
               yLabels = c("BLA", "RED", "MAG", "BLU", "YLW", "BRN"),
               #yLabels = names(MEs_order),
               ySymbols = names(MEs_order),
               colorLabels = FALSE,
               colors = blueWhiteRed(50),
...
filename=paste("modules_eigengene3.pdf",sep="")
pdf(filename, width=8, height=6)
plotEigengeneNetworks(MEs,"",marDendro=c(0,4,1,6), 
                      marHeatmap=c(3,4,1,2), cex.lab=0.8,xLabelsAngle=90)
dev.off()

# Plot the relationships among the eigengenes as png
filename=paste("modules_eigengene3.png",sep="")
...
write.csv(geneTraitSignificance, file= "geneTraitsSignif-Condition-black-updated-trait-data-q.csv")
...
write.csv(geneInfo, file = "geneInfo_Condition.csv")
write.csv(geneInfo0, file = "geneInfo-not-Condition-ordered-q.csv")

#gene screening method bsed on detailied definition of module membership
NS1=networkScreening(y=datTraits$Condition, datME=MEs_order, datExpr=datExpr,
                     oddPower=3, blockSize=1000, minimumSampleSize=4,
                     addMEy=TRUE, removeDiag=FALSE, weightESy=0.5)
write.csv(NS1, "gene-screening.csv")
...
#write.csv(geneModuleMembership, file= "geneModuleMembeship_updated_trait_data-q.csv")
write.csv(geneTraitSignificance, file= "geneTraitsSignif-AcetateC2-red-updated-trait-data-q.csv")
...
# Order modules by their significance for weight
modOrder = order(-abs(cor(MEs_order, AcetateC2, use = "p")));
...
#gene screening method bsed on detailied definition of module membership using pearson and spearman correlations
NS1=networkScreening(y=datTraits$Acetate.C2, datME=MEs_order, datExpr=datExpr,
                     oddPower=3, blockSize=1000, minimumSampleSize=4,
                     addMEy=TRUE, removeDiag=FALSE, weightESy=0.5)
...
write.csv(NS1, "gene-screening-acetatec2-spearman.csv")
...
#write.csv(geneModuleMembership, file= "geneModuleMembeship_updated_trait_data-q.csv")
write.csv(geneTraitSignificance, file= "geneTraitsSignif-AcetoneC2-ywllow-updated-trait-data-q.csv")
...
# Order modules by their significance for weight
modOrder = order(-abs(cor(MEs_order, AcetoneC2, use = "p")));
...
#gene screening method bsed on detailied definition of module membership
NS1=networkScreening(y=datTraits$Acetone.C2, datME=MEs_order, datExpr=datExpr,
                     oddPower=3, blockSize=1000, minimumSampleSize=4,
                     addMEy=TRUE, removeDiag=FALSE, weightESy=0.5)
...
NS1=networkScreening(y=datTraits$Acetone.C2, datME=MEs_order, datExpr=datExpr,
                     oddPower=3, blockSize=1000, minimumSampleSize=4,
                     addMEy=TRUE, removeDiag=FALSE, weightESy=0.5, corOptions = "use = 'p', method = 'spearman'")
write.csv(NS1, "gene-screening-acetone-spearman.csv")
...
#write.csv(geneModuleMembership, file= "geneModuleMembeship_updated_trait_data-q.csv")
write.csv(geneTraitSignificance, file= "geneTraitsSignif-DiacetylC2-blue-updated-trait-data-q.csv")
...
# Order modules by their significance for weight
modOrder = order(-abs(cor(MEs_order, DiacetylC2, use = "p")));
...
#gene screening method bsed on detailied definition of module membership
NS1=networkScreening(y=datTraits$Diacetyl.C2, datME=MEs_order, datExpr=datExpr,
                     oddPower=3, blockSize=1000, minimumSampleSize=4,
                     addMEy=TRUE, removeDiag=FALSE, weightESy=0.5)
...
NS1=networkScreening(y=datTraits$Diacetyl.C2, datME=MEs_order, datExpr=datExpr,
                     oddPower=3, blockSize=1000, minimumSampleSize=4,
                     addMEy=TRUE, removeDiag=FALSE, weightESy=0.5, corOptions = "use = 'p', method = 'spearman'")
write.csv(NS1, "gene-screening-diacetyl-spearman.csv")

###compare modules to DEG
geneModuleMembership$datExpr <- rownames(geneModuleMembership)
MMPvalue$datExpr <- rownames(MMPvalue)

black <- geneInfo0 %>%
  filter(ModuleColor == "black") %>%
  filter(datExpr != "<NA>") 

black <- merge(black, geneModuleMembership, by = "datExpr")
black <- merge(black, MMPvalue, by = "datExpr")

black.MM <- black %>%
  select(c("datExpr", "MEblack.x", "MEblack.y")) 
...
#  PreDrought_48hr 0.1320         1.0000         -               -           -          
#  Drought_0hr     0.0022         0.0087         0.0087          -           -          
#  Drought_6hr     0.0022         0.0303         0.0931          0.0931      -          
#  Drought_48hr    0.0043         0.0079         0.0087          0.4286      0.1775     
...
#  PreDrought_48hr 0.485          0.177          -               -           -          
#  Drought_0hr     0.310          0.931          0.589           -           -          
#  Drought_6hr     0.026          0.429          0.065           0.937       -          
#  Drought_48hr    0.052          0.151          0.052           0.082       0.126      
...
#             PreDrought_0hr PreDrought_6hr PreDrought_48hr Drought_0hr Drought_6hr
#PreDrought_6hr  0.792          -              -               -           -          
#  PreDrought_48hr 0.041          0.126          -               -           -          
#  Drought_0hr     0.310          0.429          0.093           -           -          
#  Drought_6hr     0.394          0.329          0.041           0.699       -          
#  Drought_48hr    0.792          1.000          0.052           0.429       0.537      
...
#               PreDrought_0hr PreDrought_6hr PreDrought_48hr Drought_0hr Drought_6hr
#PreDrought_6hr  0.0173         -              -               -           -          
#  PreDrought_48hr 0.0931         0.7922         -               -           -          
#  Drought_0hr     0.3095         0.0087         0.0649          -           -          
#  Drought_6hr     0.9372         0.1255         0.2403          0.3095      -          
#  Drought_48hr    0.2468         0.8413         0.5368          0.0519      0.3290     
...
pairwise.wilcox.test(ME.yellow.m$ME.yellow, ME.yellow.m$Condition_Time, p.adjust.method = "none")
#data:  ME.yellow.m$ME.yellow and ME.yellow.m$Condition_Time 

#                PreDrought_0hr PreDrought_6hr PreDrought_48hr Drought_0hr Drought_6hr
#PreDrought_6hr  0.0303         -              -               -           -          
#  PreDrought_48hr 0.4848         0.3290         -               -           -          
#  Drought_0hr     0.3095         0.0043         0.2403          -           -          
#  Drought_6hr     0.5887         0.0303         0.6991          0.1320      -          
#  Drought_48hr    0.7922         0.0159         0.6623          0.1775      0.7922     

#P value adjustment method: none 

pairwise.wilcox.test(ME.yellow.m$ME.yellow, ME.yellow.m$Condition, p.adjust.method = "none")
#data:  ME.yellow.m$ME.yellow and ME.yellow.m$Condition 
...
#               PreDrought_0hr PreDrought_6hr PreDrought_48hr Drought_0hr Drought_6hr
#PreDrought_6hr  0.0519         -              -               -           -          
#  PreDrought_48hr 0.0152         1.0000         -               -           -          
#  Drought_0hr     0.0022         0.0043         0.1320          -           -          
#  Drought_6hr     0.0043         0.0087         0.3095          0.3095      -          
#  Drought_48hr    0.0087         0.0159         0.3290          0.7922      0.6623     
