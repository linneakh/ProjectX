...
for (i in 1:nrow(data.13.P.long)) {
  if (isTRUE(data.13.P.long[i,1] == data.13.P.long[(i+1),1])) 
...
data.13.D.long$Time <- data.13.D.long$Time %>% replace_na('Flux')

#write.csv(data.13.D.long, "../../Output/CO2-VOCs/data.13.D.long.acetone.csv")
...
for (i in 1:nrow(data.13.D.long)) {
  if (isTRUE(data.13.D.long[i,1] == data.13.D.long[(i+1),1])) 
  {df.d.0[i,] <- spread(data.13.D.long[i:(i+1),], Time, Time_Flux)#[1,]
 }
}

#write.csv(df.d.0, "../../Output/CO2-VOCs/df.d.0.acetone.csv")
...
#write.csv(df.d, "../../Output/CO2-VOCs/df.d.acetone.csv")

#Add column for pyruvate label and add column for drought
df.d.f <- df.d %>% 
  mutate_at("Label", str_replace, ".C", "_C") %>%
...
#write.csv(df.d.f, "../../Output/CO2-VOCs/df.d.f.acetone.csv")

df.acetone <- rbind(df.p.f, df.d.f) %>%
  na.omit()

#write.csv(df.acetone, "../../Output/CO2-VOCs/df.acetone.csv")
...
data.13.P.diac.long <- data.13.P.diac %>%
  pivot_longer(cols = contains("P"),
              names_to = c("Label","Time"),
              names_sep = "_",
              values_to= "Time_Flux") %>%
...
data.13.P.diac.long$Time <- data.13.P.diac.long$Time %>% replace_na('Flux')

#write.csv(data.13.P.diac.long, "../../Output/CO2-VOCs/data.13.Plong.diacetl.csv")

# since flux and time are now in single column, with each pairs of rows containing info for single sample/timepoint,
# here we will spread each pair of rows and spread the data so flux and time become separate rows
df.p.0.diac = data.frame(matrix (NA, ncol = 3, nrow = 1))

for (i in 1:nrow(data.13.P.diac.long)) {
  if (isTRUE(data.13.P.diac.long[i,1] == data.13.P.diac.long[(i+1),1])) 
  {df.p.0.diac[i,] <- spread(data.13.P.diac.long[i:(i+1),], Time, Time_Flux)#[1,]
 }
}
#write.csv(df.p.0.diac, "../../Output/CO2-VOCs/df.p.0.diac.csv")

# erase rows with NA and name columns
df.p.diac <- df.p.0.diac %>%
  dplyr::rename(Label = X1, Flux = X2, Time = X3) %>%
  filter(Label != "NA")
#write.csv(df.p.diac, "../../Output/CO2-VOCs/df.p.diac.csv")
...
df.p.f.diac <- df.p.diac %>% 
  ...
  separate(Label, into = c("Label", "Pyr"), sep = "_") %>%
  mutate(Condition = "Pre-Drought")
#write.csv(df.p.f.diac, "../../Output/CO2-VOCs/df.p.f.diacetl.csv")

### process drought dataset
# gather dataframe 
data.13.D.diac.long <- data.13.D.diac %>%
  pivot_longer(cols = contains("P"),
               names_to = c("Label","Time"),
               names_sep = "_",
               values_to= "Time_Flux") %>%
...
data.13.D.diac.long$Time <- data.13.D.diac.long$Time %>% replace_na('Flux')

#write.csv(data.13.D.diac.long, "../../Output/CO2-VOCs/data.13.D.long.diacetyl.csv")

# since flux and time are now in single column, with each pairs of rows containing info for single sample/timepoint,
# here we will spread each pair of rows and spread the data so flux and time become separate rows
df.d.0.diac = data.frame(matrix (NA, ncol = 3, nrow = 1))

for (i in 1:nrow(data.13.D.diac.long)) {
  if (isTRUE(data.13.D.diac.long[i,1] == data.13.D.diac.long[(i+1),1])) 
  {df.d.0.diac[i,] <- spread(data.13.D.diac.long[i:(i+1),], Time, Time_Flux)#[1,]
 }
}

#write.csv(df.d.0.diac, "../../Output/CO2-VOCs/df.d.0.diacetyl.csv")

# erase rows with NA and name columns
df.d.diac <- df.d.0.diac %>%
  dplyr::rename(Label = X1, Flux = X2, Time = X3) %>%
  filter(Label != "NA")

#write.csv(df.d.diac, "../../Output/CO2-VOCs/df.d.diacetyl.csv")
...
#write.csv(df.d.f.diac, "../../Output/CO2-VOCs/df.d.f.diacetyl.csv")

df.diacetyl <- rbind(df.p.f.diac, df.d.f.diac) %>%
  na.omit()

#write.csv(df.diacetyl, "../../Output/CO2-VOCs/df.diacetyl.csv")
...
plot.Diac <- AUC.Diac %>%
  ...
  theme(text = element_text(size = 14,
        ...
        legend.title = element_blank())
plot.Diac
ggsave("../../Figures/Fig2-S1-CO2-VOCs/Fig2I-total-C-diacetyl-flux-c2-revision.png",units=c('in'),width=2, height = 4,dpi=300,plot.Diac)
...
plot <- AUC.acetate %>%
  ...
  theme(text = element_text(size = 14,
        ...
        legend.title = element_blank())
plot
ggsave("../../Figures/Fig2-S1-CO2-VOCs/Fig2G-total-C-acetate-flux-revision.png",units=c('in'),width=4, height = 4,dpi=300,plot)
...
pie.p <- ggplot(summary.allocation.p.t, aes(x="", y=perc.alloc, fill =cat.alloc )) +
  ...
  theme(legend.title = element_blank())
pie.p
#ggsave("../../Figures/Fig2-S1-CO2-VOCs/Fig2J-pie_p_bio_tca.png",units=c('in'),width=3, height = 3,dpi=300,pie.p)
