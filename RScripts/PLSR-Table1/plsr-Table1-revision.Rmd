...
```{r}
res.col.diacetyl <- c(1:3)
pls.reg1.diacetyl <- plsreg1(sub_diacetyl_pls2[,res.col.diacetyl], sub_diacetyl_pls2[,4], comps=2, crosval = TRUE)
...
```{r plsr-diacetyl-100-iterations}
#create empty data frame 
cross.val <- data.frame(matrix(ncol = 5, nrow = 1))
colnames(cross.val) <- c("CV.PRESS", "CV.RSS", "CV.Q2", "CV.LimQ2", "CV.Q2cum")
...
for (n in 1:2000) {
   if (n %% 2 ==1) {
      pls.reg1.diacetyl <- plsreg1(sub_diacetyl_pls2[,res.col.diacetyl], sub_diacetyl_pls2[,4], comps=2, crosval = TRUE)
      cross.val[n:(n+1),1:5] <- pls.reg1.diacetyl$Q2
   }
      n = n + 1
  }
...
```
## Filter to acetate and acetone flux and acetate cycling genes without time 0
```{r}
KO_acetate <- KO_interest_subset %>%
  filter(VOC == "Acetate") 
...
idx <- match(c(KO_acetate_list,flux_list), names(merged_subet_no_0))
idx <- idx[!is.na(idx)]

sub_acetate_pls <- merged_subet_no_0[,idx]

sub_acetate_pls2_no_0 <- sub_acetate_pls %>%
  mutate(tmt = ifelse(Condition == "drought", 1, 0)) %>%
  dplyr::select(-Condition, -Site)

sub_acetate_pls2_no_0
```
### acetate w/ acetate genes and acetone flux without 0
```{r}
res.col.acetate <- c(1:9, 11:12)
pls.reg1.acetate <- plsreg1(sub_acetate_pls2_no_0[,res.col.acetate], sub_acetate_pls2_no_0[,10], comps=2, crosval = TRUE)
...
KO_acetate_list <- KO_acetate$KO
flux_list <- c("log.flux.acetate", "log.flux.acetone", "log.SM", "Condition", "Site")

idx <- match(c(KO_acetate_list,flux_list), names(merged_subet_SM))
idx <- idx[!is.na(idx)]

sub_acetate_pls <- merged_subet_SM[,idx]

sub_acetate_pls2_SM <- sub_acetate_pls %>%
  mutate(tmt = ifelse(Condition == "drought", 1, 0)) %>%
  dplyr::select(-Condition, -Site)
...
```{r}
res.col.acetate <- c(1:9, 11:12)
pls.reg1.acetate <- plsreg1(sub_acetate_pls2_SM[,res.col.acetate], sub_acetate_pls2_SM[,10], comps=2, crosval = TRUE)
...
plot(pls.reg1.acetate)
```
### acetate w/ acetate genes and acetone flux with SM without low drivers
```{r}
res.col.acetate <- c(1,2,3,4,6,7,8,11,12)
pls.reg1.acetate <- plsreg1(sub_acetate_pls2_SM[,res.col.acetate], sub_acetate_pls2_SM[,10], comps=2, crosval = TRUE)
...
#create empty data frame 
cross.val <- data.frame(matrix(ncol = 17, nrow = 18))
#rownames(cross.val) <- c("it.1", "it.2", "it.3", "it.4", "it.5", "it.6", "it.7", "it.8", "it.9", "it.10",
#                         "it.11", "it.12", "it.13", "it.14", "it.15", "it.16", "it.17", "it.18")
#colnames(cross.val) <- colnames(sub_acetate_pls2_for_anova[,1:11])
...
for (n in 1:18) {
      ...
      pls.reg1.acetate <- plsreg1(plsr_minus_one[,1:11], plsr_minus_one[,12], comps=2, crosval = FALSE)
      cross.val[n,] <- pls.reg1.acetate$resid
      n = n + 1
  }

cross.val

cross.val.df.t.num <- as.numeric(unlist(cross.val))
cross.val.for.anova <- matrix(cross.val.df.t.num, nrow = 18, byrow = FALSE)
#cross.val.for.anova.df <- as.data.frame(cross.val.for.anova)
...
cross.val.for.anova.t.long <- cross.val.for.anova.t %>%
  gather(key = "iteration", value = "residuals")

res_aov <- aov(residuals ~ iteration,
  data = cross.val.for.anova.t.long
)
summary(res_aov)

ggplot(res_aov) +
  aes(x = iteration, y = residuals) +
  geom_boxplot()
#ifelse(as.integer(rownames(q2.scores)) %% 2, )
```
...
for (n in 1:2000) {
   if (n %% 2 == 1 ) {
      pls.reg1.acetone <- plsreg1(sub_acetone_pls2_SM[,res.col.acetone], sub_acetone_pls2_SM[,9], comps=2, crosval = TRUE)
      cross.val[n:(n+1),1:5] <- as.data.frame(pls.reg1.acetone$Q2)[1:2,]
   }
    n = n + 1
  }
