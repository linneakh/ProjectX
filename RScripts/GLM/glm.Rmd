...
acetone_flux <- read.csv("../../Data/CO2-VOCs/voc_raw/acetone-for-ggplot.ccsv")
...
acectate_flux_proc <- acetate_flux %>%
  ...
  select(-c("Pyruv", "Type", "Flux", "Time", "Time.hr"))
...
acetone_flux_proc <- acetone_flux %>%
  ...
  select(-c("Pyruv", "Type", "Flux", "Time", "Time.hr", "Site"))
...
merged.num <- merged %>%
  select(-c(Sample, Condition, Time.d, Site, Site.y, Site.x))

z = merged$Condition
y = merged$flux.acetone
colNames <- names(merged)[4:39]
for (i in colNames) {
  p <- ggplot(merged, aes_string(x=i, y=y)) +
  geom_point() +
...
cor(merged$K10856, merged$flux.acetone)

summary(lm(merged$flux.acetone ~ merged$K10856))
